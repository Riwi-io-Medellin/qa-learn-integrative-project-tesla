...
async function dashboard() {
  ...
  try { const d = await apiFetch('/api/users');   userList     = normalizeArrayResponse(d); } catch {}
  try { const d = await apiFetch('/api/library'); libItems     = normalizeArrayResponse(d); } catch {}
  try { const d = await apiFetch('/api/admin/test-cases'); pendingCases = normalizeArrayResponse(d); } catch {}
  try { const d = await apiFetch('/api/courses'); courseList   = normalizeArrayResponse(d); } catch {}
...
async function courses() {
  ...
  try { 
    ...
    courseList = normalizeArrayResponse(data);
  } catch {}

...
export const createEvidence = async (id_execution, data) => {
    const query = `
        INSERT INTO evidences(id_execution, type, file_url, original_filename, file_size, description)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id_evidence, id_execution, type, file_url, original_filename, file_size, description, uploaded_at
    `;
    const values = [
        id_execution,
        data.type,
        data.file_path,           // multer guarda filename → lo usamos como file_url
...
export const findEvidencesByExecution = async (id_execution) => {
    const query = `
        SELECT id_evidence, id_execution, type, file_url, original_filename, file_size, description, uploaded_at
        FROM evidences

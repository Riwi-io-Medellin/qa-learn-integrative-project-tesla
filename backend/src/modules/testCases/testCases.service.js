...
export const updateTestCase = async (id_project, id_test_case, id_user, fields) => {
    ...
    if (!VALID_TYPES.includes(fields.type))
        throw new Error(`Invalid type. Valid values: ${VALID_TYPES.join(', ')}`);
    const updated = await testCasesRepository.updateTestCase(id_test_case, id_project, fields);
    if (!updated) throw new Error('Test case not found or could not be updated');
    return updated;
};
